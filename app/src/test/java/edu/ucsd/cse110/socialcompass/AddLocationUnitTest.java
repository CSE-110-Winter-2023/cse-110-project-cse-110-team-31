...
//        assertEquals(scenario.getState(), Lifecycle.State.DESTROYED);
    ...
    @Test
    public void testSubmittingLabelWithoutLocation() {
        var scenario = ActivityScenario.launch(AddLocationsActivity.class);
        scenario.moveToState(Lifecycle.State.CREATED);
        scenario.moveToState(Lifecycle.State.STARTED);

        scenario.onActivity(activity -> {
            TextView family_label_view = (TextView) activity.findViewById(R.id.family_label_view);
            Button submit_button = (Button) activity.findViewById(R.id.submit_button);
            family_label_view.setText("test");
            submit_button.performClick();
            assertEquals(scenario.getState(), Lifecycle.State.STARTED);
            scenario.moveToState(Lifecycle.State.DESTROYED);
        });
        ...
        scenario.onActivity(activity -> {
            TextView family_label_view = (TextView) activity.findViewById(R.id.family_label_view);
            assertEquals("test", family_label_view.getText().toString());
        });
    }

    @Test
    public void testSubmittingLabelWithLocation() {
        var scenario = ActivityScenario.launch(AddLocationsActivity.class);
        scenario.moveToState(Lifecycle.State.CREATED);
        scenario.moveToState(Lifecycle.State.STARTED);
        ...
        scenario.onActivity(activity -> {
            TextView family_label_view = (TextView) activity.findViewById(R.id.family_label_view);
            TextView family_long_view = (TextView) activity.findViewById(R.id.family_long_view);
            TextView family_lat_view = (TextView) activity.findViewById(R.id.family_lat_view);
            Button submit_button = (Button) activity.findViewById(R.id.submit_button);
            family_label_view.setText("test_location");
            family_long_view.setText("2");
            family_lat_view.setText("3");
            submit_button.performClick();
        });
        ...
        scenario.onActivity(activity -> {
            ...
            assertEquals("test_location", family_label_view.getText().toString());
            assertEquals("2", family_long_view.getText().toString());
            assertEquals("3", family_lat_view.getText().toString());

        });
    }

    @Test
    public void testNotSubmittingLabel() {
        var scenario = ActivityScenario.launch(AddLocationsActivity.class);
        scenario.moveToState(Lifecycle.State.CREATED);
        scenario.moveToState(Lifecycle.State.STARTED);
        ...
        scenario.onActivity(activity -> {
            TextView family_label_view = (TextView) activity.findViewById(R.id.family_label_view);
            assertEquals("", family_label_view.getText().toString());
        });
    }
}